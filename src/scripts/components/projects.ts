    ...
    public scrollUpdate(): void {
        ...
        const inRange = (utils: { vhRelativeScroll?: () => number }) => {
            ...
            const handleTransition = (
                ...
                if (vhScrollPos >= min && vhScrollPos < min + 0.6) {
                    if (vhScrollPos >= min + 0 && vhScrollPos < min + 0.1) {
                        hero.style.transform = `rotateY(${90 - 0}deg)`;
                    ...
                    label.style.opacity = `${vhScrollPos / 0.6}`;
                } else if (vhScrollPos >= min + 0.6 && vhScrollPos < max - 0.6) {
                    hero.style.transform = 'rotateY(0deg)';
                    inRange(hero);
                    label.style.opacity = '1';
                } else {
                    if (vhScrollPos >= max - 0.6 && vhScrollPos < max - 0.5) {
                        hero.style.transform = `rotateY(${72 - 90}deg)`;
                    } else if (vhScrollPos >= max - 0.5 && vhScrollPos < max - 0.4) {
                    ...
                    label.style.opacity = `${1 - (vhScrollPos - (max - 0.6)) / 0.6}`;
                }
            };

            if (vhScrollPos >= 1 && vhScrollPos < 3) {
